...
function expiryBadge(days) {
  ...
  if (days < 0) return { label:"EXPIRED", color:"#ff4444", bg:"#2a0a0a", border:"#ff4444" };
  if (days <= 30) return { label:`${days}d left`, color:"#ff6b35", bg:"#2a1200", border:"#ff6b35" };
  if (days <= 90) return { label:`${days}d left`, color:"#f0b429", bg:"#2a1f00", border:"#f0b429" };
  return { label:`${days}d left`, color:"#4caf7d", bg:"#0a1f10", border:"#4caf7d" };
}
    ...
    <div style={{minHeight:"100vh",background:"#f7f4ef",fontFamily:"Georgia,serif",color:"#1a1a1a",margin:0}}>
      <style>{`
        * { box-sizing: border-box; }
        ...
        @media (max-width: 768px) {
          ...
          .card-grid { display: flex !important; flex-direction: column; gap: 10px; padding: 10px 12px; }
          .filter-scroll { flex-wrap: nowrap; overflow-x: auto; padding-bottom: 4px; }
          .filter-scroll::-webkit-scrollbar { display: none; }
          .controls-row { flex-wrap: nowrap !important; overflow-x: auto; }
          .hide-mobile { display: none !important; }
          .header-stats { gap: 8px !important; }
          .header-stat { padding: 8px 10px !important; min-width: 70px; }
      ...
      <div style={{background:"#1a1a2e",color:"#f7f4ef",padding:"16px 20px",borderBottom:"4px solid #c8963e"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div>
            ...
            <div style={{fontSize:"10px",color:"#8a8aaa",letterSpacing:"2px",textTransform:"uppercase",marginTop:"2px"}}>DR's Secret · Avance · Optrimax {saving && "· Saving..."}</div>
          </div>
        ...
        <div className="header-stats" style={{display:"flex",gap:"10px",marginTop:"14px",flexWrap:"wrap"}}>
          {[
            [products.filter(p=>p.status!=='sold').reduce((a,p)=>a+p.qty,0),"Units","#c8963e",""],
            [products.filter(p=>p.category==='Skincare').length,"Skincare","#e879b8","#e879b8"],
            [products.filter(p=>p.category==='Supplement').length,"Supps","#7a9aef","#7a9aef"],
            [urgentCount,"Urgent","#ff6b6b","#ff4444"],
          ].map(([n,l,color,border])=>(
            <div key={l} className="header-stat" style={{background:"rgba(255,255,255,0.07)",borderRadius:"8px",padding:"10px 14px",borderLeft:border?`3px solid ${border}`:"none"}}>
              <div className="stat-num" style={{fontSize:"22px",fontWeight:"700",color}}>{n}</div>
      ...
      <div style={{display:"flex",background:"#f0ebe0",borderBottom:"1px solid #e0d8c8",overflowX:"auto"}}>
        {[["all","All"],["Supplement","Supplements"],["Skincare","DR's Secret"]].map(([val,label])=>(
          <button key={val} onClick={()=>setCategoryFilter(val)} style={{flex:1,padding:"10px 8px",textAlign:"center",fontSize:"11px",fontWeight:"700",letterSpacing:"1px",textTransform:"uppercase",color:categoryFilter===val?"#1a1a2e":"#aaa",borderBottom:categoryFilter===val?"3px solid #c8963e":"3px solid transparent",background:"transparent",border:"none",borderBottom:categoryFilter===val?"3px solid #c8963e":"3px solid transparent",cursor:"pointer",fontFamily:"Georgia,serif",whiteSpace:"nowrap"}}>{label}</button>
        ))}
      </div>
      ...
      <div style={{padding:"10px 16px",background:"#fff",borderBottom:"1px solid #e8e0d0"}}>
        ...
        <div className="filter-scroll">
          ...
          <select className="hide-mobile" style={{border:"1px solid #d0c8b8",borderRadius:"20px",padding:"6px 12px",fontSize:"11px",fontFamily:"Georgia,serif",background:"#f0ebe0",marginLeft:"auto"}} value={sortBy} onChange={e=>setSortBy(e.target.value)}>
      ...
      {showAddForm && (
        <div style={{background:"#1a1a2e",padding:"16px 20px",borderBottom:"2px solid #c8963e"}}>
          ...
          <div className="add-grid" style={{display:"grid",gridTemplateColumns:"repeat(auto-fill, minmax(150px, 1fr))",gap:"10px"}}>
              ...
              return (
                <div key={p.id} style={{background:"#fff",borderRadius:"12px",overflow:"hidden",boxShadow:"0 1px 4px rgba(0,0,0,0.08)",borderLeft:`4px solid ${borderColor}`,opacity:p.status==='sold'?0.6:1}}>
                  <div style={{padding:"12px 14px 8px",display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                    <div style={{flex:1}}>
                      <div style={{display:"flex",gap:"6px",alignItems:"center",marginBottom:"4px",flexWrap:"wrap"}}>
                        <span style={{background:p.category==='Skincare'?"#2a0a1e":"#0a1020",color:p.category==='Skincare'?"#e879b8":"#7a9aef",padding:"2px 7px",borderRadius:"3px",fontSize:"9px",fontWeight:"700",letterSpacing:"1px"}}>{p.category==='Skincare'?'SKIN':'SUPP'}</span>
                        ...
                        <span style={{background:p.status==="sold"?"#0a2a0a":p.status==="low"?"#2a1200":"#f0ebe0",color:p.status==="sold"?"#5a9a5a":p.status==="low"?"#ff6b35":"#aaa",padding:"2px 7px",borderRadius:"3px",fontSize:"9px",fontWeight:"600"}}>{p.status}</span>
                      </div>
                  ...
                  {isEditing ? (
                    <div style={{padding:"10px 14px",background:"#faf8f4",display:"grid",gridTemplateColumns:"1fr 1fr",gap:"8px"}}>
                      {[["expiry","Expiry"],["qty","Qty"],["price","Price (RM)"],["soldTo","Sold To"],["notes","Notes"]].map(([field,label])=>(
                        <div key={field}><div style={{fontSize:"9px",color:"#aaa",textTransform:"uppercase",letterSpacing:"1px",marginBottom:"2px"}}>{label}</div><input style={{border:"1px solid #d0c8b8",borderRadius:"6px",padding:"6px 8px",fontSize:"12px",fontFamily:"Georgia,serif",background:"#fff",width:"100%"}} value={p[field]} onChange={e=>updateLocal(p.id,field,field==="qty"?+e.target.value:e.target.value)} type={field==="qty"?"number":"text"} /></div>
                      ))}
                  ...
                  <div style={{padding:"8px 14px",display:"flex",gap:"8px",borderTop:"1px solid #f0ebe0"}}>
                    {isEditing ? (
                      <><button onClick={()=>saveEdit(p)} style={{flex:1,background:"#4caf7d",border:"none",color:"#fff",borderRadius:"8px",padding:"8px",fontSize:"12px",fontWeight:"600",cursor:"pointer",fontFamily:"Georgia,serif"}}>✓ Save</button><button onClick={()=>setEditingId(null)} style={{flex:1,background:"transparent",border:"1px solid #ddd",color:"#888",borderRadius:"8px",padding:"8px",fontSize:"12px",cursor:"pointer",fontFamily:"Georgia,serif"}}>Cancel</button></>
                    ) : (
                      <><button onClick={()=>setEditingId(p.id)} style={{flex:1,background:"transparent",border:"1px solid #c8963e",color:"#c8963e",borderRadius:"8px",padding:"8px",fontSize:"12px",fontWeight:"600",cursor:"pointer",fontFamily:"Georgia,serif"}}>Edit</button><button onClick={()=>markSold(p)} style={{flex:2,background:"#5a9a5a",border:"none",color:"#fff",borderRadius:"8px",padding:"8px",fontSize:"12px",fontWeight:"600",cursor:"pointer",fontFamily:"Georgia,serif"}}>Mark Sold</button><button onClick={()=>deleteProduct(p)} style={{background:"transparent",border:"1px solid #cc4444",color:"#cc4444",borderRadius:"8px",padding:"8px 10px",fontSize:"12px",cursor:"pointer",fontFamily:"Georgia,serif"}}>✕</button></>
                    )}
                  </div>
                </div>
              );
            ...
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:"13px"}}>
              ...
              <tbody>
                {filtered.map(p => {
                  ...
                  const inp = {border:"1px solid #d0c8b8",borderRadius:"3px",padding:"4px 7px",fontSize:"12px",fontFamily:"Georgia,serif",background:"#faf8f4",width:"100%",boxSizing:"border-box"};
                  ...
                  return (
                    <tr key={p.id} style={rowBg}>
                      <td style={td}><span style={{background:p.category==='Skincare'?"#2a0a1e":"#0a1020",color:p.category==='Skincare'?"#e879b8":"#7a9aef",padding:"2px 7px",borderRadius:"3px",fontSize:"9px",fontWeight:"700",letterSpacing:"1px"}}>{p.category==='Skincare'?'SKIN':'SUPP'}</span></td>
                      ...
                      <td style={td}>{isEditing?<select style={sel} value={p.status} onChange={e=>updateLocal(p.id,"status",e.target.value)}>{STATUS_OPTIONS.map(o=><option key={o}>{o}</option>)}</select>:<span style={{background:p.status==="sold"?"#0a2a0a":p.status==="reserved"?"#1a1a2e":p.status==="low"?"#2a1200":"#f0ebe0",color:p.status==="sold"?"#5a9a5a":p.status==="reserved"?"#8a8aef":p.status==="low"?"#ff6b35":"#888",padding:"2px 7px",borderRadius:"3px",fontSize:"10px",fontWeight:"600",letterSpacing:"1px"}}>{p.status}</span>}</td>
